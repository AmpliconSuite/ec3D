...
if __name__ == "__main__":
    parameter_file = csv.writer(open('sim_para_test.csv', 'w'), delimiter='\t')
    parameter_file.writerow(['case_id', '#pinches', '#bins', 'alpha', 'beta', '#duplicated_bins', 'same_duplication'])
    case_id = 10
    for num_pinches in [1]:
        for k in [3]: # total number of bins - 250, 500, 750
            print(f'simulating structure {case_id} with duplication')
            ...
            if random.choice([0, 1]) == 0: # same duplicated substructure

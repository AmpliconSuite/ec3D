...
```
ec3D --cool <FILE> --ecdna_cycle <FILE> --resolution <INT> --output_prefix <STRING>
```
...
#### Usage
```ec3D extract_matrix [Required arguments] [Optional arguments]```
#### Required arguments
- ```--cool <FILE>``` (or ```--hic <FILE>```), Hi-C matrix, in ```*.cool``` or ```*.hic``` format.
...
#### Output of Step 1
Outputs from ```extract_matrix``` include 
- (i) the ICE normalized collapsed matrix ```*_ice_normalized.txt/npy```;
- (ii) the raw collapsed matrix ```*_raw_collapsed_matrix.txt/npy```; and
...
#### Usage
```ec3D reconstruct [Required arguments] [Optional arguments]```
#### Required arguments
- ```--matrix <FILE>```, ICE normalized, collapsed matrix in ```*.txt``` or ```*.npy``` format (```*_ice_normalized.txt/npy```) from ```extract_matrix```.
- ```--annotation <FILE>```, Annotation of bins in the input matrix (```*_annotations.bed```) from ```extract_matrix```.
...
- ```--raw_matrix <FILE>```, for generating the expanded matrix.

And finally, to work with ```*.npy``` format, you can again specify
- ```--save_npy``` to save the output numpy matrices in ```*.npy``` format.
#### Output of Step 2
After ```ec3D reconstruct``` is completed, the following files will be written to the path specified in ```--output_prefix```:
...
The capability of ec3D to expand the ecDNA Hi-C matrix additionally enables its identification of significant interactions within an expanded matrix. Given the (ICE) normalized, expanded matrix corresponding to ecDNA, the ```significant_interactions``` option provides an option to identify significant interactions from a  **circular genome**, and separate significant interactions from different causes (SV breakpoints, conformational changes, and candidate _trans_-interactions between different ecDNA molecules).
#### Usage
```ec3D significant_interactions [Required arguments] [Optional arguments]```
#### Required arguments
- ```--matrix <FILE>```, ICE normalized, expanded matrix in ```*.txt``` or ```*.npy``` format (```*_expanded_matrix.txt/npy```) from the ```reconstruct``` step.
...
Usage: ```ec3D plot_structure [Required arguments] [Optional arguments]```
#### Required arguments
- ```--structure <FILE>```, the structure reconstruction ```*_coordinates.txt/npy``` output from ```reconstruct```.
...
Another plotting option provided by ```plot_structure``` is to include the significant interactions as well as their clusters (see the right plot below), and for this purpose you will need
...
```ec3D plot_interactions [Required arguments] [Optional arguments]```
...
#### Output of Step 4-2
- Running ```ec3D plot_interactions``` will produce a copy of ```*_expanded/collapsed_matrix.png```, and another copy of ```*_expanded/collapsed_matrix.pdf``` images with the prefix specified in ```--output_prefix```. See below for examples. Top left: plot of expanded matrix; Top right: plot of expanded matrix along with significant interactions; Bottom left: plot of collapsed matrix along with significant interactions; Bottom right: plot of expanded matrix with significant interactions and additional SVs identified by AmpliconArchitect.
